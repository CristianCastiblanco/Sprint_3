    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                db.execute(
                    "UPDATE activationlink set state=? WHERE id=? and CURRENT_TIMESTAMP between created and validuntil", (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(
    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                ...
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    "UPDATE user set password=?, salt=? wher id=?", (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
